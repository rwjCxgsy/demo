...
class SubwayStation {
    ...
    draw() {
        ...
        stationCTX.fillText(name, x * GX + offset[0] + (textAlign === 'end' ? (-16) : (16)), y * GY + offset[1])
        stationCTX.restore()
    }
    updata() {
...
class Line {
    ...
    draw() {
        const {name, width, stations: list, active } = this
        // const direction = ''
        const { length } = list
        let [ ins ] = list
        let {x: x1, y: y1} = ins
        const color = ins.color
