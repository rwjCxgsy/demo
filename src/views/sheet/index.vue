 ...
 * @LastEditTime: 2020-07-01 18:31:05
 * @Description:
  ...
  <div class="sheet">
    <el-form ref="form" :model="form" label-width="80px">
      ...
      <el-form-item label="合并字段">
        <el-input
          v-model="form.key"
          placeholder="两个表格需要根据那个字段来合并"
        />
      </el-form-item>
      <el-form-item label="表格索引">
        <el-input
          v-model="form.a"
          placeholder="需要合并的第一个表格索引（默认为1）"
        />
      </el-form-item>
      <el-form-item label="表格索引">
        <el-input
          v-model="form.b"
          placeholder="需要合并的第二个表格索引（默认为2）"
        />
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async send(body) {
      this.loading = true;
      try {
        const { code, data, msg } = await fetch(
          "https://www.tiptopme.cn/utils/api/xlsx-merge",
          {
            body,
            headers: {
